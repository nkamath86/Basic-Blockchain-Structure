...
def shareInfo():
	...
	serialized_data = json.dumps(info) #info as json object

	# print str(len(str(len(serialized_data))))

	#receive info of other user as serialized json object
	data_size = s.recv(len(str(len(serialized_data)))) #receive the size of data first, assuming length to be approx imately the same
	# print str(data_size)
	
	serialized_data = s.recv(int(data_size))
	...
	s.sendall(str(len(serialized_data))) #first send the length :/
...
def makeTransaction():
	...
	if int(raw_input('Send (1) or Receive(2) ? ').strip()) == 1:
		...
		# s.sendall(data_size) 
		s.sendall(encrypted_transaction)
		s.sendall(signature)
		...
		#receive
		# data_size = s.recv(6) #receive the data size
		# print data_size
		encrypted_transaction = s.recv(256) #receive the data
